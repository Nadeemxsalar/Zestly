  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-6 pb-10 relative">
      ...
      <div className="flex justify-between items-end px-1">
        ...
        {/* 2. Grid/List Toggle */}
        <div className="flex bg-white/5 p-1 rounded-xl ring-1 ring-white/10 shadow-lg">
      ...
      <div className="relative group z-20">
        ...
        <form onSubmit={handleAddItem} className="relative bg-black/60 backdrop-blur-2xl ring-1 ring-white/10 p-1.5 rounded-[2rem] flex items-center shadow-2xl">
          
          {/* 6. Barcode Scanner Button */}
          <button type="button" className="p-3.5 text-slate-400 hover:text-orange-400 hover:bg-white/5 rounded-2xl transition-all">
          ...
          <input 
            ...
            placeholder="Add Avocados..." 
            className="flex-1 bg-transparent text-white font-medium px-2 outline-none placeholder:text-slate-500"
          />
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2">
        <button onClick={() => setFilter("all")} className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold transition-all ${filter === "all" ? "bg-white text-black shadow-lg" : "bg-white/5 text-slate-400 hover:bg-white/10"}`}>All Items ({items.length})</button>
        <button onClick={() => setFilter("urgent")} className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${filter === "urgent" ? "bg-red-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.4)]" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
          <span className="w-2 h-2 rounded-full bg-current animate-pulse"></span> Expiring Soon ({stats.urgent})
        </button>
        <button onClick={() => setFilter("low")} className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${filter === "low" ? "bg-yellow-500 text-black shadow-[0_0_15px_rgba(234,179,8,0.4)]" : "bg-yellow-500/10 text-yellow-500 border border-yellow-500/20"}`}>
