...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      // Auto-redirect hata diya hai. Bas user ko batayenge ki password galat hai.
      ...
      <div className="relative z-10 w-full sm:max-w-md h-screen sm:h-auto animate-in fade-in zoom-in-95 duration-700">
        ...
        <div className="h-full sm:h-auto bg-[#07070a] sm:bg-white/[0.03] sm:backdrop-blur-3xl p-8 sm:p-10 sm:rounded-[2.5rem] sm:border sm:border-white/[0.08] sm:shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col justify-center">
          ...
          <div className="text-left sm:text-center mb-8">
            ...
            <p className="text-slate-400 font-medium">Log in to your Zestly account</p>
          </div>
          ...
          <button 
            ...
            className="w-full bg-white hover:bg-slate-50 text-slate-900 font-bold py-4 rounded-2xl transition-all duration-300 active:scale-[0.97] mb-6 flex justify-center items-center gap-3 shadow-lg"
          >
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="group">
              ...
              <div className="flex justify-end mt-2 pr-1">
                <Link href="/forgot-password" className="text-sm font-medium text-slate-400 hover:text-orange-400 transition-colors">
                  Forgot password?
                </Link>
              </div>
            </div>
            ...
            <button 
              disabled={loading}
              className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white font-black py-4 rounded-2xl shadow-xl shadow-orange-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all mt-6"
            >
              {loading ? "Authenticating..." : "Sign In to Zestly"}
            </button>
          </form>
